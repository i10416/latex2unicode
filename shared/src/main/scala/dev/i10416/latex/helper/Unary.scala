...
object Unary {
  ...
  def makeNot(negated: String): String = {
    ...
    not.getOrElse(s, s"${s.head}\u0338${s.tail}")
  }
  ...
  def translate(command: String, param: String): String = {
    if (
      !names.contains(command) && !UnaryWithOption.HasDefault.names.contains(
        command
      )
    ) {
    ...
    suffixCmds(param)
      .orElse(negate(param))
      .orElse(UnaryWithOption.handleNoOpt(param))
      .orElse(combiningCmd(param))
      .orElse(stylesCmd(param))
      .apply(command)
  }

  def combiningCmd(param: String): PartialFunction[String, String] = {
    case cmd if isCombiningCommand(cmd) => translateCombining(cmd, param)
  }
  def stylesCmd(param: String): PartialFunction[String, String] = {
    case cmd if isStylesCommand(cmd) => translateStyles(cmd, param)
  }

  def suffixCmds(param: String): PartialFunction[String, String] = {
    case "_" | "\\textsubscript"   => makeSubscript(param)
    case "^" | "\\textsuperscript" => makeSuperScript(param)
  }
  def negate(param: String): PartialFunction[String, String] = { case "\\not" =>
    makeNot(param)
  }
}
